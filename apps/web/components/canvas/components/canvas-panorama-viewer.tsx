"use client";
/**
 * 来源：vendor/infinite-canvas（tigerowo/infinite-canvas，AGPL-3.0）— D12 画布 v2 移植（Phase D）
 * 裁剪：getProxyUrl（本项目图片 URL 可直读）；shadcn 化：无 antd 依赖。
 * 活跃 viewer 上限 4（WebGL 上下文保护）+ 失活回退静态图。
 */
...
function PanoramaSurface({ src, alt, viewerEntry }: { src: string; alt: string; viewerEntry: PanoramaViewerEntry }) {
...
export default function CanvasPanoramaViewer({ src, alt, expandOnDoubleClick = false, immersive = false, onMoveStart, onOpen }: CanvasPanoramaViewerProps) {
    ...
    return (
        <div
            className="relative h-full w-full overflow-hidden rounded-3xl bg-black/40"
            data-canvas-no-zoom
            onClick={(event) => event.stopPropagation()}
            onMouseDown={(event) => event.stopPropagation()}
            onPointerEnter={activate}
