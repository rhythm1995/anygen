 ...
 * shadcn 化：滚轮防护类名 .ant-* 换为 radix 弹层容器
 */
...
const ZOOM_GUARD_SELECTOR = "[data-canvas-no-zoom],[data-radix-popper-content-wrapper],[data-radix-popup-content],.ant-modal,.ant-popover,.ant-dropdown,.ant-select-dropdown";
...
export function InfiniteCanvas({ containerRef, viewport, tool, backgroundMode = "lines", onViewportChange, onCanvasMouseDown, onCanvasDeselect, onCanvasDoubleClick, onContextMenu, onDrop, children }: InfiniteCanvasProps) {
    ...
    const handleWheel = (event: React.WheelEvent<HTMLDivElement>) => {
        const target = event.target instanceof Element ? event.target : null;
        if (target?.closest(ZOOM_GUARD_SELECTOR)) return;
