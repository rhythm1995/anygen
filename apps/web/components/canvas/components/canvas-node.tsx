 ...
 * storageKey 依赖已替换为 assetId（见 types.ts CanvasNodeMetadata）
 */
...
import { formatBytes, formatDuration } from "@/lib/canvas-image-utils";
...
function PanoramaNodeContent(props: NodeContentRendererProps) {
    ...
    const proxyGeneratedPanorama = Boolean(props.node.metadata?.imageTaskId || props.node.metadata?.imageTaskResultId) && !props.node.metadata?.assetId;
