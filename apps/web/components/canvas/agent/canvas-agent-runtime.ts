/**
 * 来源：vendor/infinite-canvas（tigerowo/infinite-canvas，AGPL-3.0）— D12 画布 v2 移植（Phase C）
 * 内部使用合法；产品化前必须剥离（docs/CANVAS-RESEARCH.md）。
 */
...
} from "@/lib/canvas/agent-api";
import type { CanvasAgentClientConfig } from "@/lib/canvas/agent-api";
import type {
    CanvasAgentContent,
...
export async function runCanvasAgent(input: RunCanvasAgentInput): Promise<RunCanvasAgentResult> {
    ...
    let protocolMessages: CanvasAgentProtocolMessage[] = [
        ...
        { role: "user" as const, content: buildUserContent(input.userText, input.references, input.config.textModel) },
    ];
