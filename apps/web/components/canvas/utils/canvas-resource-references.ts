 ...
 * 引用标签改为内联实现（上游依赖 image-reference-prompt/seedance-video 已裁剪）
 */
...
export function assistantReferenceContentFromNode(node: CanvasNodeData): Partial<CanvasAssistantReference> | null {
    ...
    if (isCanvasImageNodeType(node.type)) return { dataUrl: content, url: undefined, assetId: node.metadata?.assetId, mimeType: node.metadata?.mimeType };
    if (node.type === CanvasNodeType.Video || node.type === CanvasNodeType.Audio) return { dataUrl: undefined, url: content, assetId: node.metadata?.assetId, mimeType: node.metadata?.mimeType };
    return null;
}
