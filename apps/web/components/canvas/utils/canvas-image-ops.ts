/**
 * 本地图像处理（D12 Phase B 尾）
 * 来源：vendor/infinite-canvas utils/canvas-image-data（AGPL-3.0）——裁剪/切分/放大/多角度全部为浏览器 canvas 算法；
 * loadImage 去掉 getProxyUrl（本项目图片 URL 同源/公开可直读）。
 */
...
export async function dataUrlToFile(dataUrl: string, name: string): Promise<File> {
    ...
    const mime = dataUrl.match(/^data:([^;]+)/)?.[1] ?? blob.type ?? "image/png";
